...
//            });
//        }
        MTIImage *image = nil;//[self.render renderToImage:pixelBuffer error:&error];
        CVPixelBufferRef returnBuffer = [self.render renderPixelBuffer:pixelBuffer error:&error];
        if (image) {
            dispatch_async(dispatch_get_main_queue(), ^{
                self.previewView.image = image;
            });
        } else {
            CVPixelBufferRetain(returnBuffer);
            dispatch_async(dispatch_get_main_queue(), ^{
                self.previewView.image = [[MTIImage alloc] initWithCVPixelBuffer:returnBuffer alphaType:MTIAlphaTypeAlphaIsOne];
                CVPixelBufferRelease(returnBuffer);
            });
        }
